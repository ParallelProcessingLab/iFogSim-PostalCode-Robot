...
/**
 * Simulation setup for Postal Code Problem
 *
 * @author Mohammad Mahdi Ghaseminya
 */
public class PostalCodeFog {
    ...
    public static void main(String[] args) {
        ...
        Log.printLine("Starting  Postal Code Problem...");
        ...
        try {
            ...
            String appId = " Postal_Code_Problem"; // identifier of the application
            ...
            Log.printLine(" Postal Code Problem finished!");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static Application createApplication(String appId, int userId) {
         ...
         */
        application.addAppModule("barcode_reader", 10); //clientModule adding module Client to the application model
        application.addAppModule("barcode_processor", 10); //processingModule adding module Concentration Calculator to the application model
        application.addAppModule("display_controller", 10); //storageModule adding module Connector to the application model

        /*
         * Connecting the application modules (vertices) in the application model (directed graph) with edges
         */
        application.addAppEdge("M-SENSOR", "barcode_reader", 3000, 500, "M-SENSOR", Tuple.UP, AppEdge.SENSOR);
        application.addAppEdge("barcode_reader", "barcode_processor", 3500, 500, "RAW_DATA", Tuple.UP, AppEdge.MODULE); // adding edge from Client to Concentration Calculator module carrying tuples of type _SENSOR
        application.addAppEdge("barcode_processor", "display_controller", 1000, 1000, "PROCESSED_DATA", Tuple.UP, AppEdge.MODULE); // adding periodic edge (period=1000ms) from Concentration Calculator to Connector module carrying tuples of type PLAYER_GAME_STATE
        application.addAppEdge("barcode_processor", "barcode_reader", 14, 500, "ACTION_COMMAND", Tuple.DOWN, AppEdge.MODULE);  // adding edge from Concentration Calculator to Client module carrying tuples of type CONCENTRATION
        application.addAppEdge("barcode_reader", "M-DISPLAY", 1000, 500, "ACTUATION_SIGNAL", Tuple.DOWN, AppEdge.ACTUATOR);  // adding edge from Client module to Display (actuator) carrying tuples of type SELF_STATE_UPDATE

        /*
         * Defining the input-output relationships (represented by selectivity) of the application modules.
         */
        application.addTupleMapping("barcode_reader", "M-SENSOR", "RAW_DATA", new FractionalSelectivity(1.0)); // 0.9 tuples of type _SENSOR are emitted by Client module per incoming tuple of type EEG
        application.addTupleMapping("barcode_processor", "RAW_DATA", "PROCESSED_DATA", new FractionalSelectivity(1.0)); // 1.0 tuples of type SELF_STATE_UPDATE are emitted by Client module per incoming tuple of type CONCENTRATION
        application.addTupleMapping("barcode_processor", "RAW_DATA", "ACTION_COMMAND", new FractionalSelectivity(1.0)); // 1.0 tuples of type CONCENTRATION are emitted by Concentration Calculator module per incoming tuple of type _SENSOR
        application.addTupleMapping("barcode_reader", "ACTION_COMMAND", "ACTUATION_SIGNAL", new FractionalSelectivity(1.0)); // 1.0 tuples of type GLOBAL_STATE_UPDATE are emitted by Client module per incoming tuple of type GLOBAL_GAME_STATE
